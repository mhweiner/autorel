...
If you don't reference the version in the command—your script just reads `process.env.NEXT_VERSION` or `$NEXT_VERSION` from the environment—you don't need `$$`. You can also put the command in `.autorel.yaml` or have your script read the env; since `--run` executes in a shell where autorel has already set `NEXT_VERSION` and `NEXT_TAG`, a single `$` in the script or YAML will resolve correctly. `NEXT_TAG` is also available (e.g. for Docker tags that use `v`).
